...
class DraftRefine():
        ...
        for epoch in range(1, opt.epoch+1):
            ...
            if epoch % opt.test_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    mean_epe = []
                    model.eval()
                    for data in tqdm(test_loader, desc='Refining motion and visibility...'):
                        pred = model(data)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            for i_eval, data in enumerate(tqdm(test_loader, desc='Predicting Vtx Corr...')):
                ...
                cv2.imwrite(
                    os.path.join(
                        config.imwrite_dir, f'{file_name[-2]}_{file_name[-1]}png'
                    ),
                    img_vis,
                )
    ...
    def gen(self):
        ...
        with torch.no_grad():
            ...
            if not os.path.exists(
                os.path.join(self.viddir, f'epoch{str(epoch_tested)}')
            ):
                os.mkdir(os.path.join(self.viddir, f'epoch{str(epoch_tested)}'))
            if not os.path.exists(
                os.path.join(self.viddir, f'epoch{str(epoch_tested)}', 'frames')
            ):
                os.mkdir(os.path.join(self.viddir, f'epoch{str(epoch_tested)}', 'frames'))
            if not os.path.exists(
                os.path.join(self.viddir, f'epoch{str(epoch_tested)}', 'videos')
            ):
                os.mkdir(os.path.join(self.viddir, f'epoch{str(epoch_tested)}', 'videos'))

            gen_frame_dir = os.path.join(
                self.viddir, f'epoch{str(epoch_tested)}', 'frames'
            )
            gen_video_dir = os.path.join(
                self.viddir, f'epoch{str(epoch_tested)}', 'videos'
            )    
